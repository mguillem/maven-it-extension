...
explained in <<assertions, Assertions chapter>>.
...
this job on its own which means you don't have to do something special here.
...
<<<
[[structuring.integrationtests]]
== Structuring Integration Tests

[[structuring.integrationtests.singletest]]
=== A Single Test Case
...
[[structuring.integrationtests.testcaseexecution]]
=== Test Case Execution
...
[[structuring.integrationtests.severaltestcases]]
=== Several Test Cases
...
[[structuring.goalsprofilesproperties]]
== Goals, Profiles, Properties and Command Line Options

[[structuring.goalsprofilesproperties.goals]]
=== Goals
...
`package` (See <<structuring.default.goals.options, for a more detailed explanation>>).
...
annotation.footnote:[Based on the usage of this annotation the parallelization is automatically deactivated,
cause Maven has never been designed to make parallel access to the maven cache possible.]
...
[[scenarios.predefined.repository.content.for.tests]]
...
[[scenarios.single.project.with.several.executions]]
...
[[test.case.execution.conditionally]]
...
The `@MavenProjectSource(sources="com/soebes/source-project")`
...
@MavenJupiterExtension
@MavenProjectSources(sources="com/soebes/source-project")
class TheIT {
...
class TheIT {
  ...
  @MavenTest
  @MavenProjectSources(sources="com/soebes/base1")
  ...
  @MavenTest
  @MavenProjectSources(sources="com/soebes/base2")
...
class TheIT {
  ...
  @Nested
  @MavenProjectSources(sources="com/soebes/base-nested")
  class NestedSetup {
    @MavenTest
    void three(MavenExecutionResult result) {
      ...
